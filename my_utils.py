...
def get_models(args, train=True, as_ensemble=False, model_file=None, leaky_relu=False, is_paral=False):
    ...
    for i in iter_m:
        ...
        # elif args.arch.lower() == 'inceptionv3':
        #     model = InceptionV3()
        else:
...
def get_loaders(args, add_gaussian=False):
    ...
    cifar_func = cifar.my_CIFAR10
    # cifar_func = mnist.my_MNIST
    # tiny_imagenet_func = tiny_imagenet.my
    ...
    #                             transform=transform_test,
    #                             download=True)
    train_dataloader = DataLoader(trainset, **kwargs)
    val_dataloader = DataLoader(testset, num_workers=4, batch_size=args.batch_size, shuffle=False,
                            pin_memory=True)  # 4, 100, F, T
    # train_dataloader = DataLoader(train_dataset, **kwargs)
    # val_dataloader = DataLoader(val_dataset, num_workers=4,batch_size=args.batch_size, shuffle=False,pin_memory=True)
...
class DistillationLoader:
    ...
    def __next__(self):
        try:
            si, sl, si_idx = next(self.seed)
            ti, tl, ti_idx = next(self.target)
            return si, sl, si_idx, ti, tl, ti_idx
...
def get_optimizers_adam(args, models):
    optimizers = []
    for model in models:
        optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-4, eps=1e-7)
        optimizers.append(optimizer)
    return optimizers

def get_schedulers_adam(args, optimizers):
    schedulers = []
    gamma = args.lr_gamma
    intervals = args.sch_intervals
    for optimizer in optimizers:
